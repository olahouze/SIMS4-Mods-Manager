...
def test_special_dependency_cases_and_wickedwhims_mapping():
    """
    Tests that only WickedWhims is defined in SPECIAL_DEPENDENCY_CASES,
    that all aliases (WW, with/without '-' or '_') match WickedWhims,
    ...
    # 1. Exactly 1 special case defined: WickedWhims
    assert len(SPECIAL_DEPENDENCY_CASES) == 1
    ww_case = SPECIAL_DEPENDENCY_CASES[0]
    assert ww_case["title"] == "WickedWhims"
    assert ww_case["remote_id"] == "3169"
    assert ww_case["url"] == "https://www.loverslab.com/files/file/3169-wickedwhims/"

    # 2. Test aliases: WW, with/without '-' or '_', casing, spaces
    variations = [
        "WW",
        "ww",
        "WickedWhims",
    ...
    for var in variations:
        res = find_special_dependency_case(var)
        assert res is not None, f"Failed to match special case for: {var}"
        assert res["title"] == "WickedWhims"
        assert res["remote_id"] == "3169"
        assert res["url"] == "https://www.loverslab.com/files/file/3169-wickedwhims/"

    # 3. Nisa's Wicked Perversions is NOT in the special cases table
    nisa_special = find_special_dependency_case("Nisa's Wicked Perversions")
    assert nisa_special is None
    assert find_special_dependency_case("NWP") is None

    # 4. Standard resolution in resolve_mod_dependencies
    db = DatabaseManager.get_instance()
    with db.get_session() as session:
        # Cleanup
        session.query(CatalogMod).filter(CatalogMod.remote_id.in_(["3169", "9443", "test_nisa"])).delete()
        session.commit()

        # Add Nisa as regular catalog mod
        nisa_cat = CatalogMod(
            source="loverslab",
            remote_id="test_nisa",
            title="Nisa's Wicked Perversions",
            page_url="https://www.loverslab.com/files/file/9443-nisas-wicked-perversions/",
        )
        session.add(nisa_cat)
        session.commit()

        raw_deps = [
            {"source": "loverslab", "remote_id": "", "title": "Wicked-Whims"},
            {"source": "loverslab", "remote_id": "", "title": "Wicked_Whims"},
            {"source": "loverslab", "remote_id": "", "title": "WW"},
...
def test_find_dependent_installed_mods():
    ...
    with db.get_session() as session:
        # Cleanup
        session.query(CatalogMod).filter(CatalogMod.remote_id.in_(["3169", "anim_pack_1", "solo_mod"])).delete()
        session.query(InstalledMod).filter(InstalledMod.remote_id.in_(["3169", "anim_pack_1", "solo_mod"])).delete()
        session.commit()

        # 1. Target Mod: WickedWhims
        target_cat = CatalogMod(
            source="loverslab",
            remote_id="3169",
            title="WickedWhims",
            page_url="https://www.loverslab.com/files/file/3169-wickedwhims/",
        )
        target_inst = InstalledMod(
            source="loverslab",
            remote_id="3169",
            title="WickedWhims",
            folder_name="loverslab_WickedWhims_100",
        )

        # 2. Dependent Mod: Animation Pack requiring WickedWhims
        anim_cat = CatalogMod(
            source="loverslab",
        ...
        anim_inst = InstalledMod(
            ...
            remote_id="anim_pack_1",
            title="Animations Pack",
            folder_name="loverslab_AnimPack_101",
        )

        # 3. Independent Mod: Solo Mod with no requirements
        solo_cat = CatalogMod(
            source="loverslab",
            remote_id="solo_mod",
            title="Solo Mod",
            page_url="https://example.com/solo",
        )
        solo_inst = InstalledMod(
            source="loverslab",
            remote_id="solo_mod",
            title="Solo Mod",
            folder_name="loverslab_Solo_102",
        )

        session.add_all([target_cat, target_inst, anim_cat, anim_inst, solo_cat, solo_inst])
        session.commit()

        # Link catalog IDs
        anim_inst.catalog_mod_id = anim_cat.id
        target_inst.catalog_mod_id = target_cat.id
        solo_inst.catalog_mod_id = solo_cat.id
        session.commit()

        # Check dependents of target_inst (WickedWhims)
        deps = find_dependent_installed_mods(target_inst.id, session)
        assert len(deps) == 1
        assert deps[0]["id"] == anim_inst.id
        assert deps[0]["title"] == "Animations Pack"
        assert deps[0]["folder_name"] == "loverslab_AnimPack_101"

        # Check dependents of solo_inst (nobody depends on it)
        solo_deps = find_dependent_installed_mods(solo_inst.id, session)
        assert len(solo_deps) == 0
